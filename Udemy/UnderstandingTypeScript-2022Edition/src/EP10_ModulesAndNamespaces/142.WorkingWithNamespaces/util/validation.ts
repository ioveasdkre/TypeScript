namespace App {
   ...
   */
  export interface ValidationTable142 {
    value: string | number;
    required?: boolean;
    minLength?: number;
    maxLength?: number;
  ...
  export function Validation142(validationTableInput: ValidationTable142) {
    ...
    if (
      ...
      isValid =
        isValid && validationTableInput.value >= validationTableInput.min;
    }
    // 檢查最大值
    if (
      ...
      isValid =
        isValid && validationTableInput.value <= validationTableInput.max;
    }
    return isValid;
  }
}
