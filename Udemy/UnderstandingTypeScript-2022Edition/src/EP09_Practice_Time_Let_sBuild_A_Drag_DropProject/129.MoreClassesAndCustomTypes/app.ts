...
type Listener = (items: Project129[]) => void;

// Project State Management 管理狀態
class ProjectState129 {
  private listeners: Listener[] = []; // 監聽使用 用來傳遞資料給其他人 避免原始資料被改動(projects)
  private projects: Project129[] = []; // 要顯示的資料
...
const projectState129 = ProjectState129.getInstance(); // 處理共用一個狀態管理 保證使用完全相同的對象

function Validation129(validationTableInput: ValidationTable129) {
...
class ProjectList129 {
  ...
  constructor(private type: "active" | "finished") {
    ...
    projectState129.addListener((projects: Project129[]) => {
      const relevantProjects = projects.filter((prj) => {
        // 判斷 當前活動狀態
        if (this.type === "active")
          return prj.status === ProjectStatus129.Active; // 回傳 是否為 Active

        return prj.status === ProjectStatus129.Finished; // 回傳 是否為 Finished
      });
...
class ProjectInput129 {
  ...
  @autobind129 // 不使用 bind的方式 裝飾器的用途之一
  private submitHandler(event: Event) {
    ...
    if (Array.isArray(userInput)) {
      ...
      projectState129.addProject(title, desc, people);
      this.clearInputs();
    }
  }
  ...
  private gatherUserInput(): [string, string, number] | void {
    ...
    const titleValidationTable: ValidationTable129 = {
      value: enteredTitle,
      required: true,
    };
    const descriptionValidationTable: ValidationTable129 = {
      value: enteredDescription,
      required: true,
      minLength: 5,
    };
    const peopleValidationTable: ValidationTable129 = {
      value: +enteredPeople,
      required: true,
      min: 1,
      max: 5,
    ...
    if (
      !Validation129(titleValidationTable) ||
      !Validation129(descriptionValidationTable) ||
      !Validation129(peopleValidationTable)
    ) {
...
const projectInput129 = new ProjectInput129();
const activeProjectList129 = new ProjectList129("active");
const finishedProjectList129 = new ProjectList129("finished");
